							...
							if (elementId !== validationConfig.passwordConfirmId)
								if (_inputElement[0].value.length < minlength || _inputElement[0].value.length > maxlength) {
									if ($el(form_group, form).nodeContains(_confirmPassword[0]).length) {
										...
										_inputElement.validate({context: g, message: `Password must be between ${minlength} and ${maxlength} characters.`});
...
const validator = new ValidateForm($el('#register-form'), form_group);
const config = validator.config.validationConfig;

config.validateEmail = false
// config.passwordConfirmId = 'confirm_password'

validator.initValidation()

console.log(validator.config.validationConfig)
// $el('#register-form').upon('submit', function (e) {
// 	e.preventDefault();
